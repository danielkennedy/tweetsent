// FIXME: Published to npm
// FIXME: Integrated with Database
// FIXME: Test since_id
  ...
  self.filterTweets = function (tweets, onDone) {
    if (_.isArray(tweets)) {
      ...
      tweets = _.filter(tweets, function (tweet) {
        return (_.has(tweet, 'id') && _.has(tweet, 'created_at') &&
          _.has(tweet, 'text') && _.has(tweet, 'retweet_count') &&
          _.has(tweet, 'user') && _.has(tweet.user, 'screen_name') &&
          _.has(tweet.user, 'followers_count'));
      });
      // Filter out blacklisted authors:
      tweets = _.reject(tweets, function (tweet) {
        return _.contains(config.twitter_author_blacklist, tweet.user.screen_name);
      });
      onDone(null, tweets);
    } else {
      onDone(new Error('tweets must be an array'));
  ...
  self.getTweets = function (onDone) {
    ...
    twitter.get('search/tweets', {
      ...
      if (err === null) {
        ...
        self.filterTweets(reply.statuses, function (err, filtered) {
          if (err === null) {
            self.reduceTweets(filtered, function (err, reduced) {
                onDone(err, reduced);
            });
          } else {
            onDone(err, reply.statuses || tweets);
          }
        });
      } else {
